...
class StringWrapper {
  ...
  isDigit(index) {
    return Number.isInteger(Number(this.chars[index]));
  }

  addSnailFishNumber(string) {
    this.chars.unshift('[');
    this.chars.push(',')
    this.chars.push(...string.split(''));
    this.chars.push(']')
  }
  ...
  _num(index) {
    ...
    return [
      Number(this.slice(start + 1, index)),
      start + 1, index
    ];
  }
  ...
  splice(start, deleteCount = 0, chars = '') {
    return this.chars.splice(start, deleteCount, ...(chars + '').split('')).join('')
  }
}
