...
input = fs.readFileSync('./input/day18-input-test.txt', 'utf-8').split('\r\n').map(v => v.trim())
...
// Solve 
const snailFishNumber = new StringWrapper(input[0]);

for (let i = 1; i < input.length; i++) {
  snailFishNumber.addSnailFishNumber(input[i])

  while (explode(snailFishNumber) || split(snailFishNumber)) {
    if (DEBUG) console.log('\n  ' + snailFishNumber.string)
  }

  console.log('\n\n    Step: ' + i + '  ' + snailFishNumber.string);
}

const result = getMagnitudeOfArray(convertToArray(snailFishNumber.string))
console.log('\n\n   Magnitude: ' + result + '\n')
