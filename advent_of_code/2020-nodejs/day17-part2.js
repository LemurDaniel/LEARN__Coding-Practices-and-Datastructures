...
const initial_states_start = [-1, -1, 0, 0];
*/

const initial_states = fs.readFileSync('./input/day17-input.txt', 'utf-8').split('\r\n').join('');
const inital_width = 8;
const initial_states_start = [-3, -3, 0, 0];
const igr = 10;  //initial_grid_size
...
/// put from all inactive cubes into grid
const initial_end = [igr, igr, igr, igr];
const initial_start = [-igr, -igr, -igr, -igr];
...
for (let i = 0; i < initial_states.length; i++) {
  const coord = [i % inital_width, Math.floor(i / inital_width), initial_states_start[2], initial_states_start[3]];
...
function vector_add(v1, v2) {
  ...
  v1[3] = parseInt(v1[3]) + parseInt(v2[3]);
  return v1;
}
...
function cycle() {
  ...
  keys.forEach(k => {
    ...
    inactive_grid[k] = rule_set[active_grid[k]](coord);
...
console.log('There are ' + count_active_in_grid() + ' active cubes in the pocket dimension after ' + cycles + ' cycles')