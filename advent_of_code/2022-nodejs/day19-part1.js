...
// NOTE:
//  node .\day19-part1.js [INPUT|TEST]
//
//  Enter:
//    'node .\day19-part1.js INPUT'    to process Todays input from 'day19-input.txt'.
//
//  Enter:
//    'node .\day19-part1.js TEST'     to process Todays Testinput from 'day19-input-test.txt'.
//    'node .\day19-part1.js'          to process Todays Testinput from 'day19-input-test.txt'.
  ...
  for (const index in Blueprint.ALL) {
    ...
    worker.on('exit', code => {
      ...
      if (results.length == Blueprint.ALL.length) {
        console.log(results)
        console.log(`\n The Sum of all Quality Levels is ${results.reduce((acc, a) => acc + a.result, 0).toLocaleString()}\n`)
        console.groupEnd()
      }
