...
Inout.solvers = [getPrerequisites_recursive];
Inout.solve();
...
function getPrerequisites_recursive(courses, current, coursesToTake = [], isIncluded = {}) {

    const iterator = !current ? Object.keys(courses) : courses[current];
    if(iterator === undefined) return 'No Ordering possible'

    for (const current of iterator) {
        if (isIncluded[current]) continue;
        isIncluded[current] = true;

        getPrerequisites_recursive(courses, current, coursesToTake, isIncluded);
        coursesToTake.push(current);
    }

    return coursesToTake;
}

// TODO: Variant with Graph